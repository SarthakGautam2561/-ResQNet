alter table sos_reports_processed add column if not exists source_project text not null default 'local';
alter table disaster_analytics add column if not exists source_project text not null default 'local';

drop index if exists disaster_analytics_district_date_idx;
create unique index if not exists disaster_analytics_district_date_idx
  on disaster_analytics (source_project, district, date_bucket);
